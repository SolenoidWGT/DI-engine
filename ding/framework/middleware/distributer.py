...
class ContextExchanger:
    ...
    def __init__(
            ...
            mq_type: str = "nng",
            debug: bool = False,
            ctx_set_method: callable = None  # User-defined set method with parameters in the form of setattr
    ) -> None:
        """
        Overview:
        ...
        self._ctx_set_method = setattr if ctx_set_method is None else ctx_set_method
    ...
    def fetch(self, ctx: "Context") -> Dict[str, Any]:
        ...
        """
        payload = {}
        # Do not use dataclasses.asdict, it will change treetensor.tensor to treetensor.object
        for field in fields(ctx):
            key = field.name
            item = getattr(ctx, key)
    ...
    def _put_env_step(self, env_step: int):
        if not task.has_role(task.role.COLLECTOR):
            if "env_step" not in self._state:
    ...
    def _put_env_episode(self, env_episode: int):
        if not task.has_role(task.role.COLLECTOR):
            if "env_episode" not in self._state:
